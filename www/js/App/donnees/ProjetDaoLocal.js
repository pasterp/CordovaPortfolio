    ...
    var dao = {
        ...
        loadLocalStorage : function () {
            if (window.localStorage.getItem('id') == null){
                this.listeProjets.push(new Projet("Roguelike", "Petit roguelike axé sur la génération procédurale.", "https://github.com/pasterp/RogueLike", "./img/roguelike.png"));
                this.listeProjets.push(new Projet("Livre dont vous êtes le héros", "Interpréteur intéractif permettant de jouer à des histoires où le joueur a le choix lors de son aventure.", "https://github.com/pasterp/LivreDontVousEtesLeHeros/", "./img/livres.jpg"));
            }else{
                id = window.localStorage.getItem('id');
                this.listeProjets = JSON.parse(window.localStorage.getItem('projets'))
            }
        },

        saveLocalStorage: function () {
            window.localStorage.setItem('id', id);
            window.localStorage.setItem('projets', JSON.stringify(this.listeProjets));
        }
    };

    dao.loadLocalStorage();
    dao.saveLocalStorage();
    return dao;
};